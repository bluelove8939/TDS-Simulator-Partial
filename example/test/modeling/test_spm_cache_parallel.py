...
if __name__ == "__main__":
    ...
    load_sessions = [
        ...
        cache.spm_load_data(ofc_addr=32, bank_idx=1, bank_oset=0, size=32, ),
        cache.spm_load_data(ofc_addr=64, bank_idx=2, bank_oset=0, size=32, ),
        cache.spm_load_data(ofc_addr=96, bank_idx=3, bank_oset=0, size=32, ),
    ]
    
    rd_wr_sessions = [
        cache.spm_read_data(bank_idx=0, bank_oset=0, size=32, ),
        cache.spm_read_data(bank_idx=1, bank_oset=0, size=32, ),
        cache.spm_read_data(bank_idx=2, bank_oset=0, size=32, ),
        cache.spm_read_data(bank_idx=3, bank_oset=0, size=32, ),
        cache.spm_write_data(bank_idx=0, bank_oset=32, size=32, data=np.ones(8, dtype=np.dtype(np.int32)), ),
        cache.spm_write_data(bank_idx=1, bank_oset=32, size=32, data=np.ones(8, dtype=np.dtype(np.int32)), ),
        cache.spm_write_data(bank_idx=2, bank_oset=32, size=32, data=np.ones(8, dtype=np.dtype(np.int32)), ),
        cache.spm_write_data(bank_idx=3, bank_oset=32, size=32, data=np.ones(8, dtype=np.dtype(np.int32)), ),
    ]
    
    store_sessions = [
        cache.spm_store_data(ofc_addr=0,  bank_idx=0, bank_oset=32, size=32, ),
        cache.spm_store_data(ofc_addr=32, bank_idx=1, bank_oset=32, size=32, ),
        cache.spm_store_data(ofc_addr=64, bank_idx=2, bank_oset=32, size=32, ),
    ...
    print("\n=== LOAD SESSIONS ===")
    for subsession in load_sessions:
    ...
    debugger.session.submit_child_session(
        subsession=cache.spm_exchange_buffering_context(),
        callbacks=[debugger.debug_callback]
    )
    ...
    print("\n=== READ/WRITE SESSIONS ===")
    for subsession in rd_wr_sessions:
        debugger.session.submit_child_session(subsession=subsession, callbacks=[debugger.debug_callback])
    ...
    print("\n=== STORE SESSIONS ===")
    for subsession in store_sessions:
        debugger.session.submit_child_session(subsession=subsession, callbacks=[debugger.debug_callback])
